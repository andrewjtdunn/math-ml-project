...
def load_shelter_data():
    ...
    df2 = df2.drop(columns=['Name', 'DateTime', 'Date of Birth', 
                            'MonthYear', 'Animal Type', 'Sex upon Outcome',
    ...
    intake_outcome_df.loc[(intake_outcome_df.loc[:,'Outcome Type'] == 'Euthanasia') | 
                          (intake_outcome_df.loc[:,'Outcome Type'] == 'Died'), 'Outcome Labels'] = 0
    intake_outcome_df.loc[((intake_outcome_df.loc[:,'Outcome Type'] == 'Adoption')
        & (
            (intake_outcome_df.loc[:,'Outcome Subtype'] != 'Foster') 
            | (intake_outcome_df.loc[:,'Outcome Subtype'].isna())
            )), 'Outcome Labels'] = 1
    intake_outcome_df.loc[intake_outcome_df.loc[:,'Outcome Subtype'] == 'Foster', 'Outcome Labels'] = 2
    intake_outcome_df.loc[intake_outcome_df.loc[:,'Outcome Type'] == 'Return to Owner', 'Outcome Labels'] = 3
    intake_outcome_df.loc[intake_outcome_df.loc[:,'Outcome Type'] == 'Transfer', 'Outcome Labels'] = 4
    intake_outcome_df.loc[intake_outcome_df.loc[:,'Outcome Type'].isna(), 'Outcome Labels'] = 5 # non-outcomes

    intake_outcome_df.loc[intake_outcome_df.loc[:,'Intake Type'] == 'Stray', 'Stray'] = 1
    intake_outcome_df.loc[intake_outcome_df.loc[:,'Intake Type'] == 'Owner Surrender', 'Owner Surrender'] = 1
    intake_outcome_df.loc[intake_outcome_df.loc[:,'Intake Type'] == 'Public Assist', 'Public Assist'] = 1
    intake_outcome_df.loc[intake_outcome_df.loc[:,'Intake Type'] == 'Abandoned', 'Abandoned'] = 1
    intake_outcome_df.loc[intake_outcome_df.loc[:,'Intake Type'] == 'Euthanasia Requested', 'Euthanasia Requested'] = 1
    intake_outcome_df['Female'] = np.where(intake_outcome_df['Sex upon Intake'].str.contains('Female'), 1, 0)
    intake_outcome_df['Intact'] = np.where(intake_outcome_df['Sex upon Intake'].str.contains('Intact'), 1, 0)
    intake_outcome_df.loc[intake_outcome_df.loc[:,'Intake Condition'] == 'Normal', 'Cond Normal'] = 1
    intake_outcome_df.loc[intake_outcome_df.loc[:,'Intake Condition'] == 'Aged', 'Cond Aged'] = 1
    intake_outcome_df.loc[((intake_outcome_df.loc[:,'Intake Condition'] == 'Med Attn') |
    ...
    intake_outcome_df.loc[(intake_outcome_df.loc[:,'Intake Condition'] == 'Other'), 'Cond Other'] = 1
    intake_outcome_df.loc[(intake_outcome_df.loc[:,'Intake Condition'] == 'Feral'), 'Cond Feral'] = 1
    intake_outcome_df.loc[(intake_outcome_df.loc[:,'Intake Condition'] == 'Neonatal'), 'Cond Neonatal'] = 1
    intake_outcome_df.loc[(intake_outcome_df.loc[:,'Intake Condition'] == 'Nursing'), 'Cond Nursing'] = 1
    intake_outcome_df.loc[(intake_outcome_df.loc[:,'Intake Condition'] == 'Pregnant'), 'Cond Pregnant'] = 1

    # clean colors (600 -> 430)
    intake_outcome_df['Color'] = intake_outcome_df['Color'].apply(lambda x: '/'.join(sorted(x.split('/')))) # standardize duplicates e.g. brown/white and white/brown -> brown/white
    intake_outcome_df['Color'] = intake_outcome_df['Color'].apply(lambda x: x.split('/')[0] if len(set(x.split('/'))) == 1 else '/'.join(sorted(set(x.split('/')))))

    # clean breeds (2600+ -> 2000)
    intake_outcome_df['Breed'] = intake_outcome_df['Breed'].apply(lambda x: '/'.join(sorted(x.split('/')))) # standardize duplicates

    # convert age
    intake_outcome_df['Age in Months'] = intake_outcome_df['Age upon Intake'].apply(convert_to_months)

    df_dog = intake_outcome_df.loc[intake_outcome_df.loc[:,'Animal Type'] == 'Dog'].copy()
    df_cat = intake_outcome_df.loc[intake_outcome_df.loc[:,'Animal Type'] == 'Cat'].copy()
    ...
    df_cat_numeric = df_cat_numeric[df_cat_numeric['Outcome Labels'] != 6] # drop outcomes we don't care about
